...
//Include the encoder part
RotaryEncoder encoder(A15,A14,5,6,100);
...
void setup()
{
    ...
    // We can set the maximum speed as 1000
...
void loop()
{
    ...
    case 2:
        ...
        /*
        stepper_Z_L.move(15350); // Z axis move down 15350
        while(stepper_Z_L.run()){
            //delayMicroseconds(50);
        }
        stepper_Z_L.stop();
        delay(2000);
        */
        Serial.println("Blade Moves to #1"); // Blade move to position #1
        while(abs(Position_float)>2){
            for(int i=0; i<10; i++){
                Position_float_old=CAN_MTS_Read(); delay(10);
            }
            //Serial.print("Position old : ");Serial.println(Position_float_old);
            stepper_blade_L.move(3838); // Half turn of 15350
            while(stepper_blade_L.run()){
            }
            //Serial.print("Blade Position : "); Serial.println(stepper_blade_L.currentPosition());
            for(int i=0; i<10; i++){
                Position_float_new=CAN_MTS_Read(); delay(10);
            }
            //Serial.print("Position new : ");Serial.println(Position_float_new);
            Position_float=Position_float_new-Position_float_old;
            //Serial.print("Position Difference : ");Serial.println(Position_float);
        }
        stepper_blade_L.stop();
    ...
    case 3:
        ...
        Lift_Caliper_Tool(stepper_Z_L);
        /*
        stepper_Z_L.move(-15350); // Z axis move up -15350
        while(stepper_Z_L.run()){
            delayMicroseconds(50);
        }
        stepper_Z_L.stop();
        */
        stepper_blade_L.move(30700); // Drive the blade into the gap each turn 15350
        while(stepper_blade_L.run()){
        }
        stepper_blade_L.stop();
        Drop_Caliper_Tool(stepper_Z_L);
    ...
    case 4:
        ...
        Position_float=CAN_MTS_Read();
/*
        stepper_Z_L.move(-15350); // Z axis move up
        while(stepper_Z_L.run()){
            delayMicroseconds(50);
        }
        stepper_Z_L.stop();

        stepper_blade_L.move(20000); // Drive the blade into the gap
        while(stepper_blade_L.run()){
        }
        stepper_blade_L.stop();

        stepper_Z_L.move(15350); // Z axis move down
        while(stepper_Z_L.run()){
            delayMicroseconds(50);
        }
        stepper_Z_L.stop();
*/
    ...
    case 5:
        ...
        stepper_Z_L.stop();
         */
        stepper_blade_L.move(30700); // Drive the blade into the gap
        while(stepper_blade_L.run()){
        }
        stepper_blade_L.stop();
        Drop_Caliper_Tool(stepper_Z_L);
